...
private:
    ...
    void get_device_name() {
        std::ifstream(DeviceNamePath) >> device_name_;
    }
    ...
    Component PinDetail() {            
        Component page = Renderer(
            ...
            [&] {
                ...
                if (g < 2) {
                    content.push_back(text("Content : " + pin_P1_P8_[head_selected_[g] * 2 + g].header));
                    content.push_back(text("Name : " + pin_P1_P8_[head_selected_[g] * 2 + g].name));
                }
                else {
                    content.push_back(text("Content : " + pin_P2_P9_[head_selected_[g] * 2 + (g-2)].header));
                    content.push_back(text("Name : " + pin_P2_P9_[head_selected_[g] * 2 + (g-2)].name));
                }
                return vbox({
                    hbox({
                        menu_P1P8_left_->Render() | size(WIDTH, GREATER_THAN , 10),
                        separator(),
                        vbox(num_8l),
                        separator(),
                        vbox(num_8r),
                        separator(),
                        menu_P1P8_right_->Render() | size(WIDTH, GREATER_THAN , 10),
                        separator(),
                        vbox({
                            content,
                        }) | flex,
                        separator(),
                        menu_P2P9_left_->Render() | size(WIDTH, GREATER_THAN , 10),
                        separator(),
                        vbox(num_9l),
                        separator(),
                        vbox(num_9r),
                        separator(),
                        menu_P2P9_right_->Render() | size(WIDTH, GREATER_THAN , 10),
                    })| border,
    ...
    Component PinConfig() {
        ...
        Component page = Renderer(Container::Horizontal({
            Container::Horizontal({
                config_p8_,
                config_p9_,
            ...
            config_tab_selected_ = (config_menu_selected_ ? num_p8 : 0) + config_selected_[config_menu_selected_];
            return hbox({
                config_p8_->Render() | size(WIDTH, GREATER_THAN , 10) | frame | vscroll_indicator,
                separator(),
                config_p9_->Render() | size(WIDTH, GREATER_THAN , 10) | frame | vscroll_indicator,
                separator(),
    ...
    Element drawBBBlack() {
        bool h_flag = false, p_flag = false, z_flag = false, s_flag = false, i_flag = false;
        int skg = 0, i = 0;
        Elements elements;
    ...
    Component tabMenu_ = Menu(&tab_names_, &tab_selected_, MenuOption::HorizontalAnimated());
