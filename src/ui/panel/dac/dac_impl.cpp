...
std::vector<std::string> FindPWMs() {
  ...
  for (const auto& it :
       std::filesystem::directory_iterator("/sys/class/pwm/")) {
...
class DACImpl : public PanelBase {
 public:
  DACImpl() {
    ...
    Component page = Renderer(
        Container::Vertical({
            pwm_radiobox,
            Container::Vertical({slider_period, dropdown_unit}),
            slider_dutyCycle,
            polarity_radiobox,
            button,
        }),
        [&] {
          return vbox({
                     ...
                     pwm_radiobox->Render(),
                     separator(),
                     hbox(slider_period->Render() | vcenter |
                              size(HEIGHT, EQUAL, 3) | xflex,
                          dropdown_unit->Render() | size(WIDTH, EQUAL, 15) |
                              size(HEIGHT, LESS_THAN, 8) | vscroll_indicator),
                     separator(),
                     slider_dutyCycle->Render(),
                     separator(),
                     hbox(text("Period: ") | bold,
                          text(std::to_string(value_period)),
                          text(unit_entries[select_unit]),
                          text(" Duty Cycle: ") | bold,
                          text(std::to_string(value_dutyCycle)), text("%")),
                     separator(),
 ...
 private:
  ...
  int selected = 0, selected_polarity = 0, value_period = 0, select_unit = 0,
      value_dutyCycle = 0;
  Component pwm_radiobox = Radiobox(&v_DAC_pin_, &selected);
  Component slider_period = Slider("Period", &value_period, 0, 100, 1);
  Component slider_dutyCycle =
      Slider("Duty Cycle(%)", &value_dutyCycle, 0, 100, 1);
